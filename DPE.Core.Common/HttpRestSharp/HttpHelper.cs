...
namespace DPE.Core.Common.HttpRestSharp
{
    /// <summary>
    ...
    public static class HttpHelper
    {
        ...
        /// <returns></returns>
        public static T PostApi<T>(string url, IDictionary<string, string> param = null, IDictionary<string, string> header= null)
        {
            ...
            {
                queest.AddHeader(item.Key, item.Value);
            }
            foreach (var item in param)
            {
                queest.AddParameter(item.Key, item.Value);
            }
