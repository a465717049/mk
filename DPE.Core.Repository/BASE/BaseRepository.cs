...
namespace DPE.Core.Repository.Base
{
    public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : class, new()
    {
        ...
        public async Task<List<TEntity>> QueryProc(string proc, SugarParameter[] parameters = null)
        {
            return await _db.Ado.UseStoredProcedure().SqlQueryAsync<TEntity>(proc, parameters);
        }
        public async Task<DataTable> QueryProcTable(string proc, SugarParameter[] parameters = null)
        {
            return await _db.Ado.UseStoredProcedure().GetDataTableAsync(proc, parameters);
        }

        public async Task<string> QueryProcSingin(string proc, SugarParameter[] parameters = null)
        {
            return (await _db.Ado.UseStoredProcedure().GetDataTableAsync(proc, parameters)).Rows[0][0].ToString();
        }

        //GetDataSetAllAsync
        public async Task<DataSet> QueryProcDataSet(string proc, SugarParameter[] parameters = null)
        {
            return await _db.Ado.UseStoredProcedure().GetDataSetAllAsync(proc, parameters);
        }

        public async Task<bool> ExecProc(string proc, SugarParameter[] parameters = null)
        {
            return await _db.Ado.UseStoredProcedure().ExecuteCommandAsync(proc, parameters)>0;
        }
    }
